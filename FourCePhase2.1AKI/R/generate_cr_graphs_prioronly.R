#' Generates serum creatinine graphs for patients with pre-admission serum creatinine values only.
...
generate_cr_graphs_prioronly <- function(siteid,base_table,obfuscation,obfuscation_level,kdigo_grade_table,ckd_valid,ckd_table,labs_cr,use_custom_output = FALSE,use_custom_output_dir = "/4ceData/Output") {
  currSiteId <- siteid
  ...
  peak_aki_vs_non_aki <- peak_trend %>% dplyr::select(patient_id,severe,time_from_peak,ratio_prioronly,value) %>% dplyr::arrange(patient_id,severe,time_from_peak,ratio_prioronly)
  peak_aki_vs_non_aki <- peak_aki_vs_non_aki %>% dplyr::group_by(patient_id,time_from_peak) %>% tidyr::fill(ratio_prioronly,value,severe) %>% dplyr::ungroup() %>% dplyr::distinct(patient_id,time_from_peak,.keep_all=TRUE)
  colnames(peak_aki_vs_non_aki) <- c("patient_id","aki","time_from_peak","ratio_prioronly","value")
  peak_aki_vs_non_aki <- peak_aki_vs_non_aki %>% dplyr::group_by(patient_id) %>% dplyr::mutate(aki = ifelse((aki == 2 | aki == 4),1,0)) %>% dplyr::ungroup()
  peak_aki_vs_non_aki_summ <- peak_aki_vs_non_aki %>% dplyr::group_by(aki,time_from_peak) %>% dplyr::summarise(mean_ratio_prioronly = mean(ratio_prioronly,na.rm=TRUE),sem_ratio_prioronly = sd(ratio_prioronly,na.rm=TRUE)/sqrt(dplyr::n()),mean_value = mean(value,na.rm=TRUE),sem_value = sd(value,na.rm=TRUE)/sqrt(dplyr::n()),n=dplyr::n()) %>% dplyr::ungroup()
  ...
  write.csv(peak_aki_vs_non_aki_summ,file=file.path(dir.output, paste0(currSiteId, "_PeakCr_AKI_vs_NonAKI_PriorCrOnly.csv")),row.names=FALSE)
  peak_aki_vs_non_aki_timeplot <- ggplot2::ggplot(peak_aki_vs_non_aki_summ,ggplot2::aes(x=time_from_peak,y=mean_ratio_prioronly,group=aki_label))+ggplot2::geom_line(ggplot2::aes(color = factor(aki_label))) + ggplot2::geom_point(ggplot2::aes(color = factor(aki_label))) + ggplot2::geom_errorbar(ggplot2::aes(ymin=mean_ratio_prioronly-sem_ratio_prioronly,ymax=mean_ratio_prioronly+sem_ratio_prioronly, color=factor(aki_label)),position=ggplot2::position_dodge(0.05))+ ggplot2::theme(legend.position="right") + ggplot2::labs(x = "Days from AKI Peak",y = "Serum Cr/Baseline Cr", color = "AKI Group") + ggplot2::xlim(-30,30) + ggplot2::ylim(0,6) + ggplot2::scale_color_manual(values=c("AKI"="#bc3c29","Non-AKI"="#0072b5")) + ggplot2::theme_minimal()
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_PeakCr_AKI_vs_NonAKI_PriorCrOnly.png")),plot=peak_aki_vs_non_aki_timeplot,width=12,height=9,units="cm")
  ...
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_PeakCr_AKI_vs_NonAKI_RawCr_PriorCrOnly.png")),plot=peak_aki_vs_non_aki_timeplot_raw,width=12,height=9,units="cm")
  ...
  peak_trend_severe <- peak_trend_severe %>% dplyr::group_by(patient_id,time_from_peak) %>% tidyr::fill(ratio_prioronly,value,severe) %>% dplyr::ungroup() %>% dplyr::distinct(patient_id,time_from_peak,.keep_all=TRUE)
  # Headers: patient_id  severe  time_from_peak  ratio_prioronly    value
  ...
  peak_cr_summ <- peak_trend_severe %>% dplyr::group_by(severe,time_from_peak) %>% dplyr::summarise(mean_ratio_prioronly = mean(ratio_prioronly,na.rm=TRUE),sem_ratio_prioronly = sd(ratio_prioronly,na.rm=TRUE)/sqrt(dplyr::n()),mean_value = mean(value,na.rm=TRUE),sem_value = sd(value,na.rm=TRUE)/sqrt(dplyr::n()),n=dplyr::n()) %>% dplyr::ungroup()
  ...
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_PeakCr_Severe_AKI_RawCr_PriorCrOnly.png")),plot=peak_cr_timeplot_raw,width=12,height=9,units="cm")
  
  # Calculate mean and SD each for each KDIGO stage
  peak_cr_kdigo_summ <- peak_trend_severe %>% dplyr::group_by(aki_kdigo_grade,time_from_peak) %>% dplyr::summarise(mean_ratio_prioronly = mean(ratio_prioronly,na.rm=TRUE),sem_ratio_prioronly = sd(ratio_prioronly,na.rm=TRUE)/sqrt(dplyr::n()),mean_value = mean(value,na.rm=TRUE),sem_value = sd(value,na.rm=TRUE)/sqrt(dplyr::n()),n=dplyr::n()) %>% dplyr::ungroup()
  ...
  write.csv(peak_cr_kdigo_summ,file=file.path(dir.output, paste0(currSiteId, "_PeakCr_KDIGOStage_AKI_PriorCrOnly.csv")),row.names=FALSE)
  # Plot the graphs
  peak_cr_kdigo_timeplot <- ggplot2::ggplot(peak_cr_kdigo_summ,ggplot2::aes(x=time_from_peak,y=mean_ratio_prioronly,group=kdigo_label))+ggplot2::geom_line(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_point(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_errorbar(ggplot2::aes(ymin=mean_ratio_prioronly-sem_ratio_prioronly,ymax=mean_ratio_prioronly+sem_ratio_prioronly,color = factor(kdigo_label)),position=ggplot2::position_dodge(0.05))+ ggplot2::theme(legend.position="right") + ggplot2::labs(x = "Days from AKI Peak",y = "Serum Cr/Baseline Cr", color = "Severity") + ggplot2::xlim(-30,30) + ggplot2::ylim(1,6) + ggplot2::scale_color_manual(values=c("No AKI"="#bc3c29","KDIGO Stage 1"="#0072b5","KDIGO Stage 2" = "#e18727","KDIGO Stage 3"="#20854e")) + ggplot2::theme_minimal()
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_PeakCr_KDIGOStage_AKI_PriorCrOnly.png")),plot=peak_cr_kdigo_timeplot,width=12,height=9,units="cm")
  peak_cr_kdigo_timeplot_raw <- ggplot2::ggplot(peak_cr_kdigo_summ,ggplot2::aes(x=time_from_peak,y=mean_value,group=kdigo_label))+ggplot2::geom_line(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_point(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_errorbar(ggplot2::aes(ymin=mean_value-sem_value,ymax=mean_value+sem_value,color = factor(kdigo_label)),position=ggplot2::position_dodge(0.05))+ ggplot2::theme(legend.position="right") + ggplot2::labs(x = "Days from AKI Peak",y = "Serum Cr (mg/dL)", color = "Severity") + ggplot2::xlim(-30,30) + ggplot2::ylim(0,6) + ggplot2::scale_color_manual(values=c("No AKI"="#bc3c29","KDIGO Stage 1"="#0072b5","KDIGO Stage 2" = "#e18727","KDIGO Stage 3"="#20854e")) + ggplot2::theme_minimal()
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_PeakCr_KDIGOStage_AKI_RawCr_PriorCrOnly.png")),plot=peak_cr_kdigo_timeplot_raw,width=12,height=9,units="cm")
  ...
  try({
    ...
    write.csv(peak_cr_kdigo_and_severe_summ,file=file.path(dir.output, paste0(currSiteId, "_PeakCr_KDIGOStage_CovidSevere_AKI_PriorCrOnly.csv")),row.names=FALSE)
    # Plot the graphs
    peak_cr_kdigo_and_severe_timeplot <- ggplot2::ggplot(peak_cr_kdigo_and_severe_summ,ggplot2::aes(x=time_from_peak,y=mean_ratio_prioronly,group=kdigo_label))+ggplot2::geom_line(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_point(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_errorbar(ggplot2::aes(ymin=mean_ratio_prioronly-sem_ratio_prioronly,ymax=mean_ratio_prioronly+sem_ratio_prioronly,color = factor(kdigo_label)),position=ggplot2::position_dodge(0.05))+ ggplot2::theme(legend.position="right") + ggplot2::labs(x = "Days from AKI Peak",y = "Serum Cr/Baseline Cr", color = "Severity") + ggplot2::xlim(-30,30) + ggplot2::ylim(0,6) + ggplot2::scale_color_manual(values=c("No AKI"="#bc3c29","KDIGO Stage 1"="#0072b5","KDIGO Stage 2" = "#e18727","KDIGO Stage 3"="#20854e")) + ggplot2::theme_minimal() + ggplot2::facet_wrap(~severe_label)
    ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_PeakCr_KDIGOStage_CovidSevere_AKI_PriorCrOnly.png")),plot=peak_cr_kdigo_and_severe_timeplot,width=12,height=8,units="cm")
    ...
    ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_PeakCr_KDIGOStage_CovidSevere_AKI_RawCr_PriorCrOnly.png")),plot=peak_cr_kdigo_and_severe_timeplot_raw,width=12,height=8,units="cm")
    ...
    if(isTRUE(ckd_present)) {
      ...
      write.csv(peak_cr_ckd_and_severe_summ,file=file.path(dir.output, paste0(currSiteId, "_PeakCr_CKD_CovidSevere_AKI_PriorCrOnly.csv")),row.names=FALSE)
      # Plot the graphs
      peak_cr_ckd_and_severe_timeplot <- ggplot2::ggplot(peak_cr_ckd_and_severe_summ,ggplot2::aes(x=time_from_peak,y=mean_ratio_prioronly,group=severe_label))+ggplot2::geom_line(ggplot2::aes(color = factor(severe_label))) + ggplot2::geom_point(ggplot2::aes(color = factor(severe_label))) + ggplot2::geom_errorbar(ggplot2::aes(ymin=mean_ratio_prioronly-sem_ratio_prioronly,ymax=mean_ratio_prioronly+sem_ratio_prioronly,color = factor(severe_label)),position=ggplot2::position_dodge(0.05))+ ggplot2::theme(legend.position="right") + ggplot2::labs(x = "Days from AKI Peak",y = "Serum Cr/Baseline Cr", color = "Severity") + ggplot2::xlim(-30,30) + ggplot2::ylim(0,6) + ggplot2::scale_color_manual(values=c("Non-severe, AKI"="#bc3c29","Non-severe, no AKI"="#0072b5","Severe, AKI" = "#e18727","Severe, no AKI"="#20854e")) + ggplot2::theme_minimal() + ggplot2::facet_wrap(~ckd_label)
      ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_PeakCr_CKD_CovidSevere_AKI_PriorCrOnly.png")),plot=peak_cr_ckd_and_severe_timeplot,width=12,height=8,units="cm")
      ...
      ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_PeakCr_CKD_CovidSevere_AKI_RawCr_PriorCrOnly.png")),plot=peak_cr_ckd_and_severe_timeplot_raw,width=12,height=8,units="cm")
  ...
  adm_to_aki_cr <- adm_to_aki_cr %>% dplyr::group_by(patient_id) %>% dplyr::mutate(ratio_prioronly = value/baseline_cr) %>% dplyr::ungroup()
  ...
  write.csv(adm_to_aki_summ,file=file.path(dir.output, paste0(currSiteId,"_CrfromAdm_Severe_AKI_PriorCrOnly.csv")),row.names=FALSE)
  
  adm_to_aki_timeplot <- ggplot2::ggplot(adm_to_aki_summ[which(adm_to_aki_summ$days_since_admission >= 0),],ggplot2::aes(x=days_since_admission,y=mean_ratio_prioronly,group=severe_label))+ggplot2::geom_line(ggplot2::aes(color = factor(severe_label))) + ggplot2::geom_point(ggplot2::aes(color = factor(severe_label))) + ggplot2::geom_errorbar(ggplot2::aes(ymin=mean_ratio_prioronly-sem_ratio_prioronly,ymax=mean_ratio_prioronly+sem_ratio_prioronly,color = factor(severe_label)),position=ggplot2::position_dodge(0.05))+ ggplot2::theme(legend.position="right") + ggplot2::labs(x = "Days from admission",y = "Serum Cr/Baseline Cr", color = "Severity") + ggplot2::xlim(-30,30) + ggplot2::ylim(0,6) + ggplot2::scale_color_manual(values=c("Non-severe, AKI"="#bc3c29","Non-severe, no AKI"="#0072b5","Severe, AKI" = "#e18727","Severe, no AKI"="#20854e")) + ggplot2::theme_minimal()
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_CrFromAdm_Severe_AKI_PriorCrOnly.png")),plot=adm_to_aki_timeplot,width=12,height=9,units="cm")
  ...
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_CrFromAdm_Severe_AKI_RawCr_PriorCrOnly.png")),plot=adm_to_aki_timeplot_raw,width=12,height=9,units="cm")
  ...
  write.csv(adm_to_aki_cr_kdigo_summ,file=file.path(dir.output, paste0(currSiteId, "_CrFromAdm_KDIGOStage_AKI_PriorCrOnly.csv")),row.names=FALSE)
  # Plot the graphs
  adm_to_aki_cr_kdigo_timeplot <- ggplot2::ggplot(adm_to_aki_cr_kdigo_summ,ggplot2::aes(x=days_since_admission,y=mean_ratio_prioronly,group=kdigo_label))+ggplot2::geom_line(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_point(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_errorbar(ggplot2::aes(ymin=mean_ratio_prioronly-sem_ratio_prioronly,ymax=mean_ratio_prioronly+sem_ratio_prioronly,color = factor(kdigo_label)),position=ggplot2::position_dodge(0.05))+ ggplot2::theme(legend.position="right") + ggplot2::labs(x = "Days since admission",y = "Serum Cr/Baseline Cr", color = "Severity") + ggplot2::xlim(-30,30) + ggplot2::ylim(0,6) + ggplot2::scale_color_manual(values=c("No AKI"="#bc3c29","KDIGO Stage 1"="#0072b5","KDIGO Stage 2" = "#e18727","KDIGO Stage 3"="#20854e")) + ggplot2::theme_minimal()
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_CrFromAdm_KDIGOStage_AKI_PriorCrOnly.png")),plot=adm_to_aki_cr_kdigo_timeplot,width=12,height=9,units="cm")
  ...
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_CrFromAdm_KDIGOStage_AKI_RawCr_PriorCrOnly.png")),plot=adm_to_aki_cr_kdigo_timeplot_raw,width=12,height=9,units="cm")
  ...
  write.csv(adm_to_aki_cr_kdigo_and_severe_summ,file=file.path(dir.output, paste0(currSiteId, "_CrFromAdm_KDIGOStage_CovidSevere_AKI_PriorCrOnly.csv")),row.names=FALSE)
  # Plot the graphs
  adm_to_aki_cr_kdigo_and_severe_timeplot <- ggplot2::ggplot(adm_to_aki_cr_kdigo_and_severe_summ,ggplot2::aes(x=days_since_admission,y=mean_ratio_prioronly,group=kdigo_label))+ggplot2::geom_line(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_point(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_errorbar(ggplot2::aes(ymin=mean_ratio_prioronly-sem_ratio_prioronly,ymax=mean_ratio_prioronly+sem_ratio_prioronly,color = factor(kdigo_label)),position=ggplot2::position_dodge(0.05))+ ggplot2::theme(legend.position="right") + ggplot2::labs(x = "Days since admission",y = "Serum Cr/Baseline Cr", color = "Severity") + ggplot2::xlim(-30,30) + ggplot2::ylim(0,6) + ggplot2::scale_color_manual(values=c("No AKI"="#bc3c29","KDIGO Stage 1"="#0072b5","KDIGO Stage 2" = "#e18727","KDIGO Stage 3"="#20854e")) + ggplot2::theme_minimal() + ggplot2::facet_wrap(~severe_label)
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_CrFromAdm_KDIGOStage_CovidSevere_AKI_PriorCrOnly.png")),plot=adm_to_aki_cr_kdigo_and_severe_timeplot,width=12,height=8,units="cm")
  ...
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_CrFromAdm_KDIGOStage_CovidSevere_AKI_RawCr_PriorCrOnly.png")),plot=adm_to_aki_cr_kdigo_and_severe_timeplot_raw,width=12,height=8,units="cm")
  
  
  # ----------------------------------------
  # Plot from start of AKI
  ...
  aki_30d_cr <- aki_30d_cr %>% dplyr::group_by(patient_id) %>% dplyr::mutate(ratio_prioronly = value/baseline_cr) %>% dplyr::ungroup()
  ...
  write.csv(aki_30d_cr_summ,file=file.path(dir.output, paste0(currSiteId,"_CrFromStart_Severe_AKI_PriorCrOnly.csv")),row.names=FALSE)
  aki_30d_cr_timeplot <- ggplot2::ggplot(aki_30d_cr_summ,ggplot2::aes(x=time_from_start,y=mean_ratio_prioronly,group=severe_label))+ggplot2::geom_line(ggplot2::aes(color = factor(severe_label))) + ggplot2::geom_point(ggplot2::aes(color = factor(severe_label))) + ggplot2::geom_errorbar(ggplot2::aes(ymin=mean_ratio_prioronly-sem_ratio_prioronly,ymax=mean_ratio_prioronly+sem_ratio_prioronly,color = factor(severe_label)),position=ggplot2::position_dodge(0.05))+ ggplot2::theme(legend.position="right") + ggplot2::labs(x = "Days from AKI start",y = "Serum Cr/Baseline Cr", color = "Severity") + ggplot2::xlim(-30,30) + ggplot2::ylim(0,6) + ggplot2::scale_color_manual(values=c("Non-severe, AKI"="#bc3c29","Non-severe, no AKI"="#0072b5","Severe, AKI" = "#e18727","Severe, no AKI"="#20854e")) + ggplot2::theme_minimal()
  ggplot2::ggsave(file=file.path(dir.output, paste0(currSiteId,"_CrFromStart_Severe_AKI_PriorCrOnly.png")),plot=aki_30d_cr_timeplot,width=12,height=9,units="cm")
  ...
  ggplot2::ggsave(file=file.path(dir.output, paste0(currSiteId,"_CrFromStart_Severe_AKI_RawCr_PriorCrOnly.png")),plot=aki_30d_cr_timeplot_raw,width=12,height=9,units="cm")
  
  # Calculate mean and SD each for each KDIGO stage
  aki_30d_cr_kdigo_summ <- aki_30d_cr %>% dplyr::group_by(aki_kdigo_grade,time_from_start) %>% dplyr::summarise(mean_ratio_prioronly = mean(ratio_prioronly,na.rm=TRUE),sem_ratio_prioronly = sd(ratio_prioronly,na.rm=TRUE)/sqrt(dplyr::n()),mean_value = mean(value,na.rm=TRUE),sem_value = sd(value,na.rm=TRUE)/sqrt(dplyr::n()),n=dplyr::n()) %>% dplyr::ungroup()
  ...
  write.csv(aki_30d_cr_kdigo_summ,file=file.path(dir.output, paste0(currSiteId, "_CrfromAdm_KDIGOStage_AKI_PriorCrOnly.csv")),row.names=FALSE)
  # Plot the graphs
  aki_30d_cr_kdigo_timeplot <- ggplot2::ggplot(aki_30d_cr_kdigo_summ,ggplot2::aes(x=time_from_start,y=mean_ratio_prioronly,group=kdigo_label))+ggplot2::geom_line(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_point(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_errorbar(ggplot2::aes(ymin=mean_ratio_prioronly-sem_ratio_prioronly,ymax=mean_ratio_prioronly+sem_ratio_prioronly,color = factor(kdigo_label)),position=ggplot2::position_dodge(0.05))+ ggplot2::theme(legend.position="right") + ggplot2::labs(x = "Time from AKI Start (days)",y = "Serum Cr/Baseline Cr", color = "Severity") + ggplot2::xlim(-30,30) + ggplot2::ylim(0,6) + ggplot2::scale_color_manual(values=c("No AKI"="#bc3c29","KDIGO Stage 1"="#0072b5","KDIGO Stage 2" = "#e18727","KDIGO Stage 3"="#20854e")) + ggplot2::theme_minimal()
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_CrfromStart_KDIGOStage_AKI_PriorCrOnly.png")),plot=aki_30d_cr_kdigo_timeplot,width=12,height=9,units="cm")
  ...
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_CrfromStart_KDIGOStage_AKI_RawCr_PriorCrOnly.png")),plot=aki_30d_cr_kdigo_timeplot_raw,width=12,height=9,units="cm")
  ...
  write.csv(aki_30d_cr_kdigo_and_severe_summ,file=file.path(dir.output, paste0(currSiteId, "_CrfromAdm_KDIGOStage_CovidSevere_AKI_PriorCrOnly.csv")),row.names=FALSE)
  # Plot the graphs
  aki_30d_cr_kdigo_and_severe_timeplot <- ggplot2::ggplot(aki_30d_cr_kdigo_and_severe_summ,ggplot2::aes(x=time_from_start,y=mean_ratio_prioronly,group=kdigo_label))+ggplot2::geom_line(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_point(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_errorbar(ggplot2::aes(ymin=mean_ratio_prioronly-sem_ratio_prioronly,ymax=mean_ratio_prioronly+sem_ratio_prioronly,color = factor(kdigo_label)),position=ggplot2::position_dodge(0.05))+ ggplot2::theme(legend.position="right") + ggplot2::labs(x = "Time from AKI Start (days)",y = "Serum Cr/Baseline Cr", color = "Severity") + ggplot2::xlim(-30,30) + ggplot2::ylim(0,6) + ggplot2::scale_color_manual(values=c("No AKI"="#bc3c29","KDIGO Stage 1"="#0072b5","KDIGO Stage 2" = "#e18727","KDIGO Stage 3"="#20854e")) + ggplot2::theme_minimal() + ggplot2::facet_wrap(~severe_label)
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_CrfromStart_KDIGOStage_CovidSevere_AKI_PriorCrOnly.png")),plot=aki_30d_cr_kdigo_and_severe_timeplot,width=12,height=8,units="cm")
  aki_30d_cr_kdigo_and_severe_timeplot_raw <- ggplot2::ggplot(aki_30d_cr_kdigo_and_severe_summ,ggplot2::aes(x=time_from_start,y=mean_value,group=kdigo_label))+ggplot2::geom_line(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_point(ggplot2::aes(color = factor(kdigo_label))) + ggplot2::geom_errorbar(ggplot2::aes(ymin=mean_value-sem_value,ymax=mean_value+sem_value,color = factor(kdigo_label)),position=ggplot2::position_dodge(0.05))+ ggplot2::theme(legend.position="right") + ggplot2::labs(x = "Time from AKI Start (days)",y = "Serum Cr (mg/dL)", color = "Severity") + ggplot2::xlim(-30,30) + ggplot2::ylim(0,4) + ggplot2::scale_color_manual(values=c("No AKI"="#bc3c29","KDIGO Stage 1"="#0072b5","KDIGO Stage 2" = "#e18727","KDIGO Stage 3"="#20854e")) + ggplot2::theme_minimal() + ggplot2::facet_wrap(~severe_label)
  ggplot2::ggsave(filename=file.path(dir.output, paste0(currSiteId,"_CrfromStart_KDIGOStage_CovidSevere_AKI_RawCr_PriorCrOnly.png")),plot=aki_30d_cr_kdigo_and_severe_timeplot_raw,width=12,height=8,units="cm")
