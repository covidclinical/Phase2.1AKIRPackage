...
runAnalysis <- function(is_obfuscated=TRUE,factor_cutoff = 5,restrict_models = FALSE) {
    ...
    peak_trend <- peak_trend %>% dplyr::group_by(patient_id) %>% dplyr::mutate(ratio = value/first_baseline_cr) %>% tidyr::fill(severe) %>% dplyr::ungroup() %>% dplyr::distinct()
    ...
    peak_aki_vs_non_aki <- peak_aki_vs_non_aki %>% dplyr::group_by(patient_id,time_from_peak) %>% tidyr::fill(ratio) %>% dplyr::ungroup() %>% dplyr::distinct(patient_id,time_from_peak,.keep_all=TRUE)
