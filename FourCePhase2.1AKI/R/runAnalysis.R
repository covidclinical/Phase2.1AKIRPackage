...
runAnalysis <- function(is_obfuscated=TRUE,factor_cutoff = 5,restrict_models = FALSE, docker = TRUE, input = "/4ceData/Input", siteid_nodocker = "" ) {
    ...
    if(isTRUE(cirrhosis_present)) {
        ...
        if(isTRUE(inr_present)) {
            ...
            ggplot2::ggsave(file=file.path(getProjectOutputDirectory(),paste0(currSiteId,"Cirrhosis"), paste0(currSiteId,"_CrFromStart_MELD_AKI.png")),plot=aki_start_meld_timeplot,width=12,height=9,units="cm")
            ...
            ggplot2::ggsave(file=file.path(getProjectOutputDirectory(),paste0(currSiteId,"Cirrhosis"), paste0(currSiteId,"_CrFromStart_AKI_CLD_only.png")),plot=aki_start_cld_timeplot,width=12,height=9,units="cm")
            message("At this point, if there are no errors, graphs and CSV files for normalised creatinine of AKI vs non-AKI patients, plotted from start of AKI/creatinine increase, should have been generated.")
        }
    }
